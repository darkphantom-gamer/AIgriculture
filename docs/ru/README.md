...
**Система мониторинга умной фермы с открытым исходным кодом для Raspberry Pi.**
Следите за влажностью почвы, автоматизируйте полив, обнаруживайте болезни, определяйте готовность к сбору урожая, получайте все оповещения и общайтесь со своей фермой с помощью FLORA AI — всё это с одной веб-панели.
...
| `ADMIN_PASS` | Стойкий пароль | (вы решаете) — если оставить пустым, при первом запуске выводится случайный пароль |
| `DB_USER` / `DB_PASS` / `DB_NAME` | Учётные данные MariaDB / MySQL | (на ваш выбор — см. раздел «База данных») |
...
**Камера безопасности** и **камера FarmMonitor** (сканы болезней / зрелости) принимают одни и те же форматы источника: RPi CSI, USB, RTSP IP или HTTP-MJPEG. Источник выбирается для каждой камеры отдельно — флагом CLI или переменной окружения.
...
# Комбинируйте: USB-камера безопасности + IP-камера FarmMonitor (напр. камера теплицы)
python main.py --security-cam /dev/video0 \
               --farm-cam   rtsp://greenhouse:5554/live
```

**Оба** флага принимают: `rpi` / `csi` (RPi CSI через OpenCV), `/dev/videoN` (USB), целое число (индекс камеры), `rtsp://…` (IP RTSP), `http://…` (IP MJPEG). Менять код для новых камер не нужно — достаточно сменить флаг или переменную окружения.
...
Если HAT на самом деле не подключён, `main-hailo.py` записывает предупреждение в лог и продолжает работать как обычно. Между двумя скриптами можно переключаться в любой момент, не трогая конфигурацию или базу данных.
